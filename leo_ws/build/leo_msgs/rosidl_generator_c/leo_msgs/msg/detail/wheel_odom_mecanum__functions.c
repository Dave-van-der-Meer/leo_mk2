  ...
  leo_msgs__msg__WheelOdomMecanum__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    leo_msgs__msg__WheelOdomMecanum * data =
      (leo_msgs__msg__WheelOdomMecanum *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!leo_msgs__msg__WheelOdomMecanum__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          leo_msgs__msg__WheelOdomMecanum__fini(&data[i]);
        }
        free(data);
        return false;
      }
